...
export function Badge({ children, tone = 'coral' }: { children: ReactNode; tone?: 'coral'|'mint'|'sky'|'stone' }) { const styles={coral:'bg-coral-50 text-coral-700',mint:'bg-mint-50 text-mint-600',sky:'bg-skysoft-50 text-sky-700',stone:'bg-stone-100 text-stone-600'}; return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${styles[tone]}`}>{children}</span> }
