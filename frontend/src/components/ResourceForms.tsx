...
import type { City, Destination, Expense, Inspiration, ItineraryItem, NewItem, Place, Reservation, RouteLeg, TransportMode, Trip } from '../types'
...
export function ItineraryForm({ item, onSave, onCancel, reservations, places, cities, tripId }: SaveProps<ItineraryItem> & { reservations: Reservation[]; places: Place[]; cities: City[]; tripId: number }) {
  const [data,setData]=useState<NewItem<ItineraryItem>>(item ? { ...item } : { trip_id:tripId,title:'',date:today(),start_time:'09:00',end_time:null,type:'景点',location:'',note:'',reservation_id:null,place_id:null,map_url:'',image_url:'',city_id:cities[0]?.id||null })
...
export function ExpenseForm({ item, onSave, onCancel, itinerary, reservations, tripCurrency, tripId }: SaveProps<Expense> & { itinerary: ItineraryItem[]; reservations: Reservation[]; tripCurrency: string; tripId:number }) {
  const [data,setData]=useState<NewItem<Expense>>(item ? { ...item } : {trip_id:tripId,title:'',amount:0,currency:tripCurrency,date:today(),category:'餐饮',payment_method:'',note:'',is_split:false,itinerary_id:null,reservation_id:null}); const s=useSubmit(onSave); const change=(key:keyof typeof data,value:unknown)=>setData(v=>({...v,[key]:value}))
...
export function TripForm({ item, onSave, onCancel }: SaveProps<Trip>) {
  ...
  const [data,setData]=useState<NewItem<Trip>>(item?{...item}:{name:'',start_date:start.toISOString().slice(0,10),end_date:end.toISOString().slice(0,10),total_budget:0,currency:'CNY'});const s=useSubmit(onSave);const change=(key:keyof typeof data,value:unknown)=>setData(v=>({...v,[key]:value}))
  return <FormShell onSubmit={e=>{e.preventDefault();void s.submit(data)}} saving={s.saving} error={s.error} onCancel={onCancel}>
    <div className="sm:col-span-2"><FormInput required label="旅程名称" placeholder="例如：法国与英国 12 日游" value={data.name} onChange={e=>change('name',e.target.value)}/></div><FormInput required type="date" label="开始日期" value={data.start_date} onChange={e=>change('start_date',e.target.value)}/><FormInput required type="date" label="结束日期" value={data.end_date} onChange={e=>change('end_date',e.target.value)}/><FormInput required type="number" min="0" step="0.01" label="总预算" value={data.total_budget} onChange={e=>change('total_budget',Number(e.target.value))}/><FormSelect label="预算货币" value={data.currency} onChange={e=>change('currency',e.target.value)}>{currencyOptions}</FormSelect>
  </FormShell>
}
...
export function DestinationForm({ item, onSave, onCancel, tripId, destinations }: SaveProps<Destination> & { tripId:number; destinations:Destination[] }) {
  const [data,setData]=useState<NewItem<Destination>>(item?{...item}:{trip_id:tripId,name:'',type:'国家',code:'',order_index:destinations.length,parent_id:null,note:''});const s=useSubmit(onSave);const change=(key:keyof typeof data,value:unknown)=>setData(v=>({...v,[key]:value}))
