...
- 行前清单提供可跨旅程复用的基础随身、洗漱、摄影、药品和出发前待办模板；打开模板后可逐项编辑、增删，再确认添加，已存在项目不会重复写入。
- 设置页可生成适合分享的公开攻略 PDF，仅包含逐日行程、路线、地点和准备清单，自动排除预约号、附件、预算、支出及私人备注。

## 使用 Docker 启动（推荐）

需要安装并启动 Docker Desktop。在统一仓库的 `web` 目录执行：

```powershell
docker compose up --build -d
```

启动完成后访问 `http://localhost:8080`；如果 `web/.env` 覆盖了端口，则使用其中的 `TRAVEL_PLANNER_PORT`。网页和 API 由同一个地址提供，Nginx 会把 `/api` 请求转发给 FastAPI 容器。
...
- 点击“导出公开攻略”可预览适合分享的版本，再通过浏览器“另存为 PDF”。
